...
```{r}
sample_dir <- paste(experiment_dir, "sample/", sep = "")
sample_files <- length(list.files(sample_dir))
...
```{r}
library(OpenImageR)
feature.hog <- function(sample_dir, data_name, set_name, export = T, cell, orientation){
...
for (i in 1:sample_files){
    ...
    cropped[[i]] <- train_data[[i]][1:200, 1:200, ]
    resized[[i]] <- resize(cropped[[i]], 64, 64)
}
  
# extract hog feature (cell and orientation numbers may be changed with tuning)
hog <- matrix(NA, nrow = sample_files, ncol = 192)
for (i in 1:sample_files){
    hog <- matrix(HOG(resized[[i]], cells = 8, orientations = 8), 
                  nrow = sample_files, ncol = 192)
  }
...
label_train_sample <- read.table(paste(experiment_dir, 
                                       "sample_train_label.txt", sep = ""), header = F)
...
hog_train_data[, order(names(hog_train_data))]
hog_train_data$pet <- as.factor(hog_train_data$pet)
...
rf_fit <- randomForest(hog_train_data[, -1], hog_train_data[, 1], 
                          ntree = 500, mtry = 10)
yhat_rf <- predict(rf_fit, TESTDATA, type = "class")
