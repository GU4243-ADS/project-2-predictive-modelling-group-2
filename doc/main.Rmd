...
# Model values fir xgboost (only one parameter):
model_values <- list()
for(i in 1:5){
...
model_values_rf <- list()
k = 1
for(i in seq(100, 500, 100)){
  for(j in seq(50, 100, 10)){
    model_values_rf[[k]] = list(ntree = i, mtry = j)
    k = k + 1
  }
}
...
# model = "gbm" # choose which model to train
model = "xgboost"
...
if(run.cv){
  err_cv_rf <- array(dim = c(length(model_values_rf), 2))
  for(k in 1:length(model_values_rf)){
    cat("k=", k, "\n")
    err_cv_rf[k,] <- cv.function(dat_train, label_train, model_values_rf[[k]], model, K)
  }
  save(err_cv_rf, file = paste0("../output/err_cv_", model, ".RData"))
}
...
model_best_rf <- model_values_rf[1]
if(run.cv){
  model_best_rf <- model_values_rf[which.min(err_cv_rf[, 1])]
}

par_best_rf <- list(ntree = model_best_rf, mtry = model_best_rf)
