...
packages.used=c("gbm", "knn", "xgboost", "randomForest", "ggplot2")
...
model_values <- list()
for(i in 1:5){
  model_values[[i]] = list(max_depth = i)
}
...
for(i in seq(300, 500, 100)){
  for(j in seq(50, 100, 10)){
    model_values[[k]] = list(max_depth = i, ntrees = j)
    k = k + 1
  }
}
...
if(run.feature.train){
 tm_feature_train <- system.time(dat_train <- feature(img_train_dir, indexes=train_indexes,
                                                       method="rgb_feature", export = FALSE))
}
...
image_1 <- data.frame(bin_nb=seq.int(dim(dat_train)[2]), freq=dat_train[1,])

ggplot(image_1, aes(image_1$bin_nb, image_1$freq)) +
...
install.packages("imhist")
library(imhist)

img <- readImage(paste0("../data/pets/train/",  "pet", 1, ".jpg"))
dim(img)
hist(img[,,1])
hist(img[,,2])
hist(img[,,3])
...
freqs_1 <- as.data.frame(table(
  factor(findInterval(img_as_rgb[,,1], rBin), levels=1:nR), 
...
# Visualizing the RGB feature: we create an array of size 4*4*total nb of bins
# each subarray of size 4*4 will contain the color of a corresponding bin
colors = matrix(NA, 1, nR*nG*nB)
k = 0
for (iR in 1:length(rBin)){
  for (iG in 1:length(gBin)){
    for (iB in 1:length(bBin)){
      colors[1, k] = rBin[iR] * gBin[iG] * bBin[iB]
      k = k + 1
    }
  }
}

# array(c(img,img,img),dim = c(nrow(img),ncol(img),3))

array(NA, dim=(4, 1440, 3))

m = matrix(runif(100),10,10)
par(mar=c(0, 0, 0, 0))
image(m, useRaster=TRUE, axes=FALSE)

frise <- data.matrix(freqs_1[c("Var1", "Var2", "Var3")])
dim(frise)

frise
image(data.matrix(freqs_1[c("Var1", "Var2", "Var3")]))


frise2 = matrix(NA, dim=c(4, 1440, 3))
dim(frise2)
plot(Image(frise, dim=c(1,1440)))
